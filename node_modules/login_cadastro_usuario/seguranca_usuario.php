<?php
/* php.ini
session.cookie_httponly=1;
session.cookie_secure=1;
session.use_only_cookies=1; */

//ini_set('session.cookie_secure', '1'); // só funciona se o site utilizar https
ini_set('session.cookie_httponly', '1');
ini_set('sesssion.use_only_cookies', '1');
ini_set('session.use_strict_mode', '1');

date_default_timezone_set("America/Bahia");

require_once 'conexao.php';

define('TEXT_SEGURANCA', 'W4r/N0Mor3Trouble@&St0pWar@');

if(!isset($marcaTempoCache)) {
    session_cache_expire(10);
    $marcaTempoCache = session_cache_expire();
}else {
    session_cache_expire($marcaTempoCache);
}
        
session_start();


// aqui renova o id da sessa caso tenha passado o tempo
if($marcaTempoCache <= 3) {
    session_regenerate_id();
    /*
     * ainda vou modificar essa renovacao de sessao
     */
}

// cria um token de sessao
$token_sessao = hash('sha512', $_SERVER['REMOTE_ADDR'].$_SERVER['HTTP_USER_AGENT'].TEXT_SEGURANCA.session_id());

/* verifica se existe algum token da sessao existe
 * caso exista, ele verifica se é igual ao da sessao existente
 */
if(isset($_SESSION['logado'],$_SESSION['token_user'],$_SESSION['email'])) {
    
    try {
        $con = getConexao();
        if($con === null) {
            header('Location: ./pagefound.php');
        }
        $con->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);

        $sql_verifica_token = 'SELECT token_session FROM cliente WHERE id = (SELECT id FROM cliente WHERE email = :email);';

        $stmt = $con->prepare($sql_verifica_token);

        $stmt->bindParam(':email', $_SESSION['email']);

        if($stmt->execute()) {
            $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
            
            // se nao retornar nada é pq o usuario nao existe
            if(count($result[0]) === 0) {
                $con = null;
                header('Location: ../login_cadastro_usuario/logout_usuario.php');
            }

            $token_usuario = $result[0]['token_session'];
            
            if(!hash_equals($token_sessao, $token_usuario)) {
                header('Location: ../login_cadastro_usuario/logout_usuario.php');
            }
            $con = null;
        }else {
            $con = null;
            header('Location: ../login_cadastro_usuario/logout_usuario.php');
        }

    } catch (PDOException $ex) {
        echo $e->getCode(); echo $e->getMessage();
    }

}else {
    $_SESSION['logado'] = false;
    if(isset($token_usuario)) {
        unset($token_usuario);
    }
}
