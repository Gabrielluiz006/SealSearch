...
try {
    ...
    if(!empty($alteraSenhaNova) && strlen($alteraSenhaNova) < 8) {
    ...
    // verifica se existe alguma variavel vazia
    if(!empty($alterar) && !empty($alteraNome) && !empty($alteraEmail) && !empty($alteraSenhaAntiga) && !empty($alteraCpf)) {
        /*
        ...
        if($stmt->execute()) {
            ...
            if(($stmt->rowCount() === $qtdParamSql || count($result[0] === $qtdParamSql)) && hash_equals($result[0]['senha'], $alteraSenhaAntiga)) {
                ...
                // se o campo de senha nova nao estiver vazio a senha é alterada
                $alteraSenhaNova = (!empty($alteraSenhaNova)) ? hash('sha512', $alteraSenhaNova.$hash) : $alteraSenhaAntiga;
        ...
        $_SESSION['msgDeVerificacao'] = '* Preencha todos os campos';
        $con = null;
        header('Location: ../PP1_FRONT/Alterar.php');
    }
