...
// deu tudo certo?
if ($response != null && $response->success && !$logado) {
...
try {
    ...
    if(isset($email,$senha,$login) && $email != '' && $senha != ''){
        ...
        $sql = 'SELECT id,nome,cpf,email,senha,data_nascimento,hash_master FROM cliente WHERE id = (SELECT id FROM cliente WHERE email = :email);';
        $qtdParamSql = 7;
